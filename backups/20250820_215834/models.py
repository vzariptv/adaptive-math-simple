...
class StudentEvaluationLog(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    # Ссылка на запуск оценки (batch header)
    evaluation_run_id = db.Column(db.Integer, db.ForeignKey('student_evaluation_runs.id'))
    user_id = db.Column(
        db.Integer,
        db.ForeignKey('users.id', ondelete='CASCADE'),
        nullable=False
    ...
    # Рассчитанные метрики (расширенный набор)
    accuracy = db.Column(db.Float)
    time_score = db.Column(db.Float)          # Новый нормированный скор времени (0..1)
    avg_time = db.Column(db.Float)            # Медиана/среднее времени в секундах (агрегат)
    progress = db.Column(db.Float)            # Существующее поле (оставляем для совместимости)
    motivation = db.Column(db.Float)          # Существующее поле (оставляем для совместимости)
    progress_score = db.Column(db.Float)      # Новый нормированный прогресс (0..1)
    motivation_score = db.Column(db.Float)    # Новый нормированный мотив (0..1)
    total_score = db.Column(db.Float)

    # Решение по уровню
    level_before = db.Column(db.String(10))  # 'low', 'medium', 'high', 'mastered'
    level_after = db.Column(db.String(10))   # 'low', 'medium', 'high', 'mastered'
    level_change = db.Column(db.String(10))  # 'up', 'down', 'stay', 'mastered'

    # Агрегаты за период
    tasks_total = db.Column(db.Integer)
    tasks_solved = db.Column(db.Integer)
    attempts_total = db.Column(db.Integer)
    a1 = db.Column(db.Integer)   # решено с 1-й попытки
    a2 = db.Column(db.Integer)   # решено со 2-й попытки
    a3 = db.Column(db.Integer)   # решено с 3-й попытки

    # Трассировка вычислений
    calc_trace = db.Column(db.JSON)
    ...
    __table_args__ = (
        db.UniqueConstraint('evaluation_run_id', 'user_id', 'topic_id', name='uq_eval_run_user_topic'),
        db.Index('ix_eval_log_user_topic_decided', 'user_id', 'topic_id', 'decided_at'),
    )
    
    def __repr__(self):
        return f'<StudentEvaluationLog user_id={self.user_id} topic_id={self.topic_id} {self.level}>'

class StudentEvaluationRun(db.Model):
    """Заголовок запуска пакетной оценки (batch header)"""
    __tablename__ = 'student_evaluation_runs'
    ...
    __table_args__ = (
        db.Index('ix_eval_run_created_at', 'created_at'),
    )
...
class EvaluationSystemConfig(db.Model):
    ...
    engagement_weight_alpha = db.Column(db.Float, default=0.667)
